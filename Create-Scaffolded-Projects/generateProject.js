...
Provide just the detailed scaffolded project guide (in markdown format) as the output.`
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      // create a file for each project with name as the idea, taking care of whitespace and special characters
      fs.writeFileSync(`${outputPath}/${idea['idea'].split(' ').join('_').replace(/[^a-zA-Z0-9]/g, '')}.md`, text);
